...
public:
	...
	static const char player_markers[3]; //棋盘格子的状态，. X O

	ConnectFourState(int num_rows_, int num_cols_,int** board_, int disable_row_, int disable_col_)
		: player_to_move(1),
		  disable_row(disable_row_),
		  disable_col(disable_col_),
	      num_rows(num_rows_),
	      num_cols(num_cols_),
		  last_col(-1),
		  last_row(-1)
	{ 
		...
		char convert2char [3] = {'.', 'O', 'X'};
		for(int row = 0;row < num_rows;++row)
			for(int col = 0;col<num_cols;++col)	
				board[row][col] = convert2char[board_[row][col]];
	}
	...
	std::vector<Move> get_moves() const
	{
		...
		moves.reserve(num_cols); //设置容量
		for (int col = 0; col < num_cols; ++col) {
			if (board[0][col] == player_markers[0]) {
				if(col == disable_col)	{
					int row = num_rows - 1;
					while (board[row][col] != player_markers[0]) row--;
					if(row == disable_row) continue;
				}
